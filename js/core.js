!function(a) {
    ...
    var c = {
        defaults: function(c, d) {
            ...
            if ("undefined" == typeof psSettings) {
                ...
                psSettings = a.extend(psSettings, d);
            return psSettings
        },
        init: function(d) {
        ...
        ctbHistoryPage: function(b, c, d) {
            if (a("html").is("[data-ks-history-pushed]")) {
                ...
                $("a.ajax").each(function() {
                    if ($(this).attr("href") == g || $(this).attr("href") == '')
    ...
    var e = {
        trigger: function(b, c, d, f, g) {
            if (c) {
                ...
                if (h())
                    a(document).on("touchstart", ".abl", function() {
                        window.history.back()
                    ...
                    a(document).on("click", ".abl", function() {
                        window.history.back()
        ...
        ksLoadPage: function(b, c, d, f, g) {
            ...
            if ("" != d) {
                ...
                a.ajax({
                    ...
                    success: function(a) {
                        if (b.bouncingBoxes)
                            e.ksFadeSiblings(b, c, a, f, g);
                        else
                            e.ksPositionAndPrepare(b, c, a, f, g)
                    },
                    dataType: "html"
                })
            } else
        ...
        ksAddreloadboxIn: function(b, c) {
            ...
            if (b.movereloadboxClasses)
                d.after("<" + f.toLowerCase() + ' class="ks-ctb-box-in ' + ("undefined" != typeof e ? e : "") + '" id="' + c + '"></' + f.toLowerCase() + ">");
            else
                d.after("<" + f.toLowerCase() + ' class="ks-ctb-box-in" id="' + c + '"></' + f.toLowerCase() + ">");
        ...
        ksPositionAndPrepare: function(b, c, d, f, g) {
            ...
            reloadboxRightAbsolute = j.left + parseFloat(l) + k - parseFloat(m),
            $reloadboxIn = a("#" + f),
            ...
            if (reloadboxInContents = void 0 != a(d).filter("#" + i).html()) {
                if (b.loadBox)
                    var t = a(d).filter(b.loadBox);
                else if (loadSelector)
                    var t = a(d).filter(loadSelector);
                else
                    var t = a(d).filter("#" + i);
                ...
                if (b.loadBox)
                    var t = a(d).find(b.loadBox);
                else if (loadSelector)
                    var t = a(d).find(loadSelector);
                else
                    var t = a(d).find("#" + i);
                reloadboxInContents = t.html();
                var u = t.attr("class")
    ...
    a.coretemp = function(b) {
        if (c[b])
            return c[b].apply(a(window), Array.prototype.slice.call(arguments, 1));
        else if ("object" === typeof b || !b)
            return c.init.apply(a(window), arguments, false);
        else
            a.error("Method " + b + " does not exist on jQuery.coretemp")
    }
    ;
